...
public class XYGraphWidget extends Widget {
    ...
    private void drawTrigger(Canvas canvas) {
        ...
        Point a = new Point((int) xOffset, (int) yOffset);
        Point b = new Point((int) (xOffset), (int) ((triggerTriangleMid * 2) + yOffset));
        Point c = new Point((int) (triggerTriangleHeight + xOffset), (int) (triggerTriangleMid + yOffset));
...
//                        labelPaint);
    ...
    public Paint getTriggerPaint() {
        return triggerLevel.getTriggerPaint();
    }

    public void setTriggerLevel(double level) {

        if (level > plot.getCalculatedMaxY().doubleValue()) {
            level = plot.getCalculatedMaxY().doubleValue();
        } else if (level < plot.getCalculatedMinY().doubleValue()) {
            level = plot.getCalculatedMinY().doubleValue();
        }
